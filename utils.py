...
def get_dataset(path, buffer_size, batch_size):
	...
	img_ds = list_ds.prefetch(tf.data.experimental.AUTOTUNE).shuffle(buffer_size).map(parse_image, num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(batch_size)
	return img_ds
